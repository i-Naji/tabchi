...
function get_admin ()
	if redis:exists('botBOT-IDadmin') then
     return true
  else
...
function find_link(text)
	if text:match("https://telegram.me/joinchat/%S+") or text:match("https://t.me/joinchat/%S+") or text:match("https://telegram.dog/joinchat/%S+") then
		...
		for link in text:gmatch("(https://telegram.me/joinchat/%S+)") do
			print("newlink")
			if  (not redis:sismember("botBOT-IDsavedlinks", link or not redis:sismember("botBOT-IDwaitelinks", link)) then
		...
		if msg.content_.ID == "MessageText" then
			...
			if is_naji(msg) then
				...
				elseif text:match("^بروزرسانی ربات$") then
					...
					local text,ok = io.open("bot.lua",'r'):read('*a'):gsub("BOT%-ID",4)
				...
				elseif text:match("^(تازه سازی)$")then
					...
					return send(msg.chat_id_,msg.id_,"<i>تازه‌سازی آمار تبلیغ‌گر شماره </i><code> 4 </code> با موفقیت انجام شد.")
